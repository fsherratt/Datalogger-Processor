...
function [deviceData] = preProcessDataFile(split, struct, devices, resampleFrequency, applyCalibration, fixAlignment, remapAxis)
...
for i = 1:length(devices)
    ...
    deviceData(i).accel_mag = vecnorm(deviceData(i).accel, 2, 2);
...
for i = 1:length(deviceData)
    ...
    deviceData(i).accel_mag = deviceData(i).accel_mag(1:minSamples, :);
...
if fixAlignment
    for i = 1:length(deviceData)
        skew = skewness(deviceData(i).gyro(:, 3)); % Skewness of saggital plane
        
        if (strcmp(deviceData(i).friendly,  'l_ankle') && skew < 0) || ...
           (strcmp(deviceData(i).friendly,  'r_ankle')  && skew > 0)
            
            fix_mat = [1, 0, 0; 0, -1, 0; 0, 0, -1];
            
            deviceData(i).accel = deviceData(i).accel * fix_mat;
            deviceData(i).gyro = deviceData(i).gyro * fix_mat;
            deviceData(i).magn = deviceData(i).magn * fix_mat;
        end
...
if remapAxis
    for i = 1:length(deviceData)
        map = [1, 0, 0; 0, 0, -1; 0, 1, 0]; %x=z, y=y, z=x
        deviceData(i).accel = deviceData(i).accel * map;
        deviceData(i).gyro = deviceData(i).gyro * map;
        deviceData(i).magn = deviceData(i).magn * map;
    end
end
